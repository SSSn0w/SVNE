...
namespace SVNE.GUI {
    class Slider : Clickable {
        ...
        public void MouseDown(RenderWindow window) {
            ...
            else if(handle.Position.X < bar.Position.X - 1) {
                x = (int)(bar.Position.X);
                handle.Position = new Vector2f(bar.Position.X, handle.Position.Y);
            }
            else {
                x = (int)(Mouse.GetPosition(window).X - handle.Size.X / 2);
                handle.Position = new Vector2f(Mouse.GetPosition(window).X - handle.Size.X / 2, handle.Position.Y);
            }
        }
